...
# التوكنات مشفرة
ENCRYPTED_BOT_TOKEN = encrypt("8700905522:AAE30w5iFr8jmhIRf_eE0EpSAmk6j1lMfn8")
ENCRYPTED_ADMIN_CHANNEL = encrypt("-1004479419959")
ENCRYPTED_ADMIN_PASS = encrypt("T13AHA990POL")

# فك التشفير عند التشغيل
BOT_TOKEN = decrypt(ENCRYPTED_BOT_TOKEN)
ADMIN_CHANNEL_ID = decrypt(ENCRYPTED_ADMIN_CHANNEL)
...
def default_catalog():
    ...
    # ===== الألعاب =====
    add("g1","📁 PUBG MOBILE","games",None,"folder", description="لعبة ببجي موبايل")
...
def get_admin_main_panel(db):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("➕ إضافة قسم (مع وصف)",callback_data="adm#add_category"),InlineKeyboardButton("➕ إضافة منتج (مع وصف)",callback_data="adm#add_product")],
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ud.get('awaiting_add_balance'):
        try:
            ...
            await update.message.reply_text("❌ الصيغة غير صحيحة! استخدم: `آيدي|المبلغ` — حاول مجدداً", reply_markup=CANCEL_BTN)
            return
    ...
    if ud.get('awaiting_search_bot_order'):
        ...
        for oid, o in found[:5]:
            ...
            msg = (
                f"🤖 **طلب بوت** `{oid}`\n"
                f"👤 الزبون: `{o.get('user_id')}`\n"
                f"💬 التواصل: {safe_md(o.get('contact',''))}\n"
                f"📝 الوصف: {safe_md(o.get('desc',''))}\n"
                f"🖥️ السيرفر: {safe_md(o.get('srv_name',''))}\n"
                f"💰 السعر: {price_txt}\n"
                f"📌 الحالة: {o.get('status')}\n"
                f"🗒️ التفاصيل المحفوظة:\n{safe_md(o.get('details') or 'لا يوجد')}"
            )
            buttons = []
            if o.get('file_id'):
    ...
    # ===== إضافة قسم (مع وصف وتأكيد) =====
    if ud.get('awaiting_add_category'):
        try:
            parts = text.split('|')
            parent_raw, section, name, description = parts[0].strip(), parts[1].strip(), parts[2].strip(), parts[3].strip() if len(parts) > 3 else ""
            ...
            ud['confirm_add_category'] = True
            
            confirm_text = (
                f"📋 **تفاصيل القسم الجديد:**\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"📌 الاسم: `{name}`\n"
                f"📂 التصنيف: `{section}`\n"
                f"📎 الأب: `{parent if parent else 'جذر'}`\n"
            ...
            await update.message.reply_text(
                confirm_text,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("✅ تأكيد", callback_data="confirm_category_add")],
                    [InlineKeyboardButton("❌ إلغاء", callback_data="cancel_flow")]
                ])
            )
        except Exception as e:
            await update.message.reply_text(
                f"❌ الصيغة غير صحيحة! استخدم:\n"
                f"`parent_id_او_root|games_او_cards_او_numbers|الاسم|الوصف`\n\n"
                f"مثال: `root|games|لعبة جديدة|وصف القسم`",
                reply_markup=CANCEL_BTN
            )
        return

    # ===== إضافة منتج (مع وصف وتأكيد) =====
    if ud.get('awaiting_add_product'):
        try:
            parts = text.split('|')
            parent, kind, name, price, description = parts[0].strip(), parts[1].strip(), parts[2].strip(), float(parts[3].strip()), parts[4].strip() if len(parts) > 4 else ""
            ...
            ud['confirm_add_product'] = True
            
            confirm_text = (
                f"📋 **تفاصيل المنتج الجديد:**\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"📌 الاسم: `{name}`\n"
                f"💰 السعر: ${price:.2f}\n"
                f"🏷️ النوع: `{kind}`\n"
            ...
            await update.message.reply_text(
                confirm_text,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("✅ تأكيد", callback_data="confirm_product_add")],
                    [InlineKeyboardButton("❌ إلغاء", callback_data="cancel_flow")]
                ])
            )
        except Exception as e:
            await update.message.reply_text(
                f"❌ الصيغة غير صحيحة! استخدم:\n"
                f"`parent_id|kind|الاسم|السعر|الوصف`\n\n"
                f"kind: game_code / card / whatsapp_number / telegram_number\n"
                f"مثال: `g3|game_code|كود 3200 شدة|45.5|كود شحن ببجي`",
                reply_markup=CANCEL_BTN
            )
        return
    ...
    if ud.get('awaiting_edit_price'):
        try:
            ...
            node['name'] = base + f" ~ {price}$"
            save_db(db)
            await update.message.reply_text(f"✅ تم تعديل سعر `{nid}` إلى {price}$ وتفعيله.")
        except:
            await update.message.reply_text("❌ الصيغة غير صحيحة! استخدم: `node_id|السعر_الجديد`.", reply_markup=CANCEL_BTN)
            return
    ...
    if ud.get('awaiting_delete_node'):
        ...
        node['deleted'] = True
        save_db(db)
        await update.message.reply_text(f"🗑️ تم حذف `{nid}` (يمكن استرجاعه لاحقاً).")
    ...
    if ud.get('awaiting_charge'):
        try:
            ...
            await update.message.reply_text(
                ...
                f"💳 **معلومات التحويل:**\n"
                f"🏦 شام كوس: `{SYRIA_CASH_NUMBER}`\n"
                f"👤 الاسم: {SYRIA_CASH_NAME}\n\n"
                f"📤 أرسل الآن **صورة الوصل** أو **اكتب رقم/مرجع العملية** لتأكيد طلبك:",
                reply_markup=CANCEL_BTN
            )
        except:
            await update.message.reply_text("❌ اكتب رقماً صحيحاً! ", reply_markup=CANCEL_BTN)
    ...
    if ud.get('awaiting_refund'):
        try:
            ...
            await update.message.reply_text(
                f"💰 المبلغ: {amount} {'$' if currency=='usd' else 'ل.س'} = **${usd_amount:.2f}**\n\n"
                f"📝 **للاسترجاع:** سيُطلب منك إدخال رقم شام كوس + اسمك الكامل\n\n"
                f"هل تريد تأكيد طلب الاسترجاع؟",
                reply_markup=btn
            )
        except:
            await update.message.reply_text("❌ اكتب رقماً صحيحاً! ", reply_markup=CANCEL_BTN)
    ...
    if ud.get('awaiting_refund_info'):
        refund_info = text
        ud['refund_info'] = refund_info
        ud['awaiting_refund_info'] = False
    ...
    if ud.get('awaiting_phone_amount'):
        try:
            ...
            await update.message.reply_text(
                ...
                f"📞 الرقم: {safe_md(ud.get('phone_number',''))}\n"
                f"📶 الشبكة: {ud.get('card_type')}\n"
                f"💰 المبلغ: {amount:,.0f} ل.س (${usd_amount:.2f})\n\n"
    ...
    if ud.get('awaiting_bot_price'):
        ...
        if not m:
            await update.message.reply_text("❌ لم أتمكن من إيجاد رقم بالسعر! اكتب مثلاً: `10$` أو `10` — حاول مجدداً.", reply_markup=CANCEL_BTN)
            return
        ...
        pay_btn = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"✅ موافقة ودفع ${price:.2f}", callback_data=f"bot_pay#{order_id}")]
        ])
        await context.bot.send_message(
            target_id,
            f"💰 **السعر المتفق عليه لطلب البوت:** ${price:.2f}\n"
            f"اضغط الزر أدناه للموافقة والدفع من محفظتك:",
    ...
    # ===== تسليم كود/رقم للزبون (من الأدمن) =====
    if ud.get('awaiting_delivery_code'):
        ...
        delivery_text = (
            f"✅ **تم تفعيل طلبك!**\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🎁 المنتج: {safe_md(order.get('item_name','رقم/بطاقة'))}\n"
...
async def handle_photo_and_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ud.get('awaiting_bot_file') and update.message.document:
        ...
        await update.message.reply_text(
            f"✅ تم إرسال الملف إلى {target_id} وتم حفظه لطلب `{order_id}` لإعادة الإرسال لاحقاً."
        )
        return
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("resend_botfile#"):
        ...
        try:
            await context.bot.send_document(
                order['user_id'],
                order['file_id'],
                caption="📂 تفضل ملف البوت الخاص بك مجدداً!"
            )
    ...
    if data == "open_panel":
        ...
        await query.edit_message_text(
            "🛸 **لوحة التحكم الإدارية الخارقة**",
            reply_markup=get_admin_main_panel(db)
        )
        return

    # ===== تأكيد إضافة قسم =====
    if data == "confirm_category_add":
        if not is_admin(db, user_id):
            await query.answer("❌ غير مصرح لك", show_alert=True)
            return
        
        cat_data = ud.get('pending_category')
        if not cat_data:
            await query.edit_message_text("❌ انتهت المهلة، حاول مجدداً.")
            return
        ...
        if cat_data['parent']:
            db['catalog'][cat_data['parent']]['children'].append(nid)
        else:
            db['catalog_roots'][cat_data['section']].append(nid)
        
        save_db(db)
        ud.pop('pending_category', None)
        ud['confirm_add_category'] = False
        ...
        await context.bot.send_message(
            ...
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🆔 المعرف: `{nid}`\n"
            f"📌 الاسم: {cat_data['name']}\n"
            f"📝 الوصف: {cat_data.get('description', 'لا يوجد')}\n"
            f"👤 أضيف بواسطة: `{user_id}`"
        )
        
        await query.edit_message_text(f"✅ تم إضافة القسم [{cat_data['name']}] بمعرف `{nid}`")
        return

    # ===== تأكيد إضافة منتج =====
    if data == "confirm_product_add":
        if not is_admin(db, user_id):
            await query.answer("❌ غير مصرح لك", show_alert=True)
            return
        
        prod_data = ud.get('pending_product')
        if not prod_data:
            await query.edit_message_text("❌ انتهت المهلة، حاول مجدداً.")
            return
        ...
        nid = new_node_id(db, "x")
        db['catalog'][nid] = {
            "name": f"🎯 {prod_data['name']} ~ {prod_data['price']}$",
            "section": db['catalog'][prod_data['parent']]['section'],
            "parent": prod_data['parent'],
            "type": "product",
            "kind": prod_data['kind'],
        ...
        db['catalog'][prod_data['parent']]['children'].append(nid)
        save_db(db)
        ud.pop('pending_product', None)
        ud['confirm_add_product'] = False
        ...
        await context.bot.send_message(
            ...
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🆔 المعرف: `{nid}`\n"
            f"📌 الاسم: {prod_data['name']}\n"
            f"💰 السعر: ${prod_data['price']:.2f}\n"
            f"📝 الوصف: {prod_data.get('description', 'لا يوجد')}\n"
            f"👤 أضيف بواسطة: `{user_id}`"
        )
        
        await query.edit_message_text(f"✅ تم إضافة المنتج [{prod_data['name']}] بمعرف `{nid}`")
        return
    ...
    if data.startswith("adm#"):
        ...
        if action == "stats":
            ...
            await query.edit_message_text(
                ...
                f"📦 الطلبات المعلقة الآن: {total_orders}\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"🛒 عمليات شراء مكتملة: {s.get('purchases',0)}\n"
                f"💸 عمليات استرجاع: {s.get('refunds',0)}\n"
                f"💳 عمليات إيداع: {s.get('deposits',0)}\n"
        ...
        if action == "pending_list":
            ...
            for oid, o in list(orders.items())[:25]:
                lines.append(f"`{oid}` — {o.get('type')} — {o.get('user_id')} — {safe_md(str(o.get('item_name', o.get('amount', ''))))}")
            await query.edit_message_text("\n".join(lines))
            return
        ...
        if action == "sub_balance":
            ...
            await query.edit_message_text("✍️ اكتب: `آيدي|المبلغ` لخصمه ", reply_markup=CANCEL_BTN)
            return
        ...
        if action == "edit_rate":
            ...
            await query.edit_message_text(
                f"📈 سعر الصرف الحالي: {db.get('exchange_rate',13800):,} ل.س\n✍️ اكتب السعر الجديد :",
                reply_markup=CANCEL_BTN
            )
            return
        ...
        if action == "add_category":
            ...
            await query.edit_message_text(
                "✍️ اكتب بالصيغة:\n`parent_id_او_root|games_او_cards_او_numbers|الاسم|الوصف`\n\n"
                "مثال (قسم رئيسي جديد): `root|games|لعبة جديدة|وصف القسم`\n"
                "مثال (مجلد فرعي داخل g1): `g1|games|مجلد فرعي|وصف المجلد`",
                reply_markup=CANCEL_BTN
            )
            return
        ...
        if action == "add_product":
            ...
            await query.edit_message_text(
                "✍️ اكتب بالصيغة:\n`parent_id|kind|الاسم|السعر|الوصف`\n\n"
                "kind: game_code / card / whatsapp_number / telegram_number\n"
                "مثال: `g3|game_code|كود 3200 شدة|45.5|كود شحن ببجي`",
                reply_markup=CANCEL_BTN
            )
            return
        ...
        if action == "edit_price":
            ...
            await query.edit_message_text(
                "✍️ اكتب بالصيغة: `node_id|السعر_الجديد`\nمثال: `g7|24.5`\n",
                reply_markup=CANCEL_BTN
            )
            return
        ...
        if action == "toggle":
            ...
            await query.edit_message_text(
                "✍️ اكتب معرف العنصر (node_id) لتبديل حالته تفعيل/تعطيل :",
                reply_markup=CANCEL_BTN
            )
            return
        ...
        if action == "delete_node":
            ...
            await query.edit_message_text(
                "✍️ اكتب معرف العنصر (node_id) لحذفه (قابل للاسترجاع) :",
                reply_markup=CANCEL_BTN
            )
            return
        ...
        if action == "restore_node":
            ...
            await query.edit_message_text(
                "✍️ اكتب معرف العنصر (node_id) لاسترجاعه :",
                reply_markup=CANCEL_BTN
            )
            return
        ...
        if action == "search_bot_order":
            ...
            await query.edit_message_text(
                "✍️ اكتب رقم طلب البوت (order_id) أو آيدي الزبون للبحث عنه:",
                reply_markup=CANCEL_BTN
            )
            return
        ...
        if action == "list_admins":
            ...
            for a in admins:
                name = safe_md(db['users'].get(a, {}).get('name', 'غير معروف'))
                lines.append(f"`{a}` — {name}")
            await query.edit_message_text("\n".join(lines) if admins else "لا يوجد أدمنية مصادقين بعد.")
            return
        ...
        if action == "export_users":
            ...
            await context.bot.send_document(
                chat_id=user_id,
                document=users_data.encode('utf-8'),
    ...
    if data == "echo#payment":
        if not is_admin(db, user_id):
            ...
            return
        await query.edit_message_text(
            f"💳 **إعدادات الدفع والتحويل**\n\n🏦 شام كوس: `{SYRIA_CASH_NUMBER}`\n👤 الاسم: {SYRIA_CASH_NAME}\n\nللتعديل: افتح الكود وعدل المتغيرات.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔙 رجوع للإعدادات", callback_data="echo#main")]
            ])
        )
    ...
    if data.startswith("nav#"):
        ...
        await query.edit_message_text(
            f"📁 **{safe_md(node['name'])}**",
            reply_markup=render_listing(db, node['children'], back_cb_for(node), f"nav#{nid}", page)
        )
        return
    ...
    if data.startswith("buy#"):
        ...
        if balance < node['price']:
            await query.edit_message_text(f"❌ رصيدك (${balance:.2f}) لا يكفي لشراء [{safe_md(node['name'])}]!")
            return
    ...
    if data == "confirm_refund":
        ...
        if balance < usd_amount:
            ...
            return
        clear_awaiting(ud)
        ud['awaiting_refund_info'] = True
        await query.edit_message_text(
            f"📝 **أدخل معلومات شام كوس لتحويل المبلغ:**\n\n💰 المبلغ: ${usd_amount:.2f}\n\n✍️ اكتب: `رقم_شام_كوس|الاسم_الكامل`\n\nمثال: `0999999999|أحمد محمد`",
            reply_markup=CANCEL_BTN
        )
        return
    ...
    if data.startswith("order_ok#"):
        ...
        await notify_admin_dm(
            context,
            f"✍️ **اكتب الآن هنا** الكود/الرقم/المعلومات لتسليمها للزبون (طلب {order_id} — {safe_md(order.get('item_name',''))}):"
        )
        return

    if data.startswith("order_no#"):
        order_id = data.split('#')[1]
        db['pending_orders'].pop(order_id, None)
        save_db(db)
    ...
    if data.startswith("phone#"):
        ...
        await query.edit_message_text(
            f"✍️ **أدخل رقم الهاتف** ({card_type.upper()}):",
            reply_markup=CANCEL_BTN
        )
        return
    ...
    if data.startswith("charge#"):
        ...
        await query.edit_message_text(
            f"✍️ **أدخل المبلغ** {'بالدولار' if currency == 'usd' else 'بالليرة السورية'}:",
            reply_markup=CANCEL_BTN
        )
        return
    ...
    if data.startswith("refund#"):
        ...
        await query.edit_message_text(
            f"✍️ **أدخل المبلغ** {'بالدولار' if currency == 'usd' else 'بالليرة السورية'} المراد استرجاعه:",
            reply_markup=CANCEL_BTN
        )
        return
    ...
    if data.startswith("bot_price#"):
        ...
        await query.edit_message_text(
            f"📩 تحقق من رسائلك الخاصة مع البوت لكتابة السعر (طلب {parts[2]})."
        )
        await notify_admin_dm(
            context,
            f"✍️ اكتب السعر الآن هنا للمستخدم `{parts[1]}` (طلب {parts[2]})، مثال: `10$` أو `10`:"
        )
        return
    ...
    if data.startswith("bot_time#"):
        ...
        await query.edit_message_text(
            f"📩 تحقق من رسائلك الخاصة مع البوت لكتابة الوقت (طلب {parts[2]})."
        )
        await notify_admin_dm(
            context,
            f"✍️ اكتب الوقت المتوقع الآن هنا للمستخدم `{parts[1]}` (طلب {parts[2]}):"
        )
        return

    # ===== تفاصيل/ملاحظات البوت (من الأدمن) =====
    if data.startswith("bot_notes#"):
        ...
        await query.edit_message_text(
            f"📩 تحقق من رسائلك الخاصة مع البوت لكتابة التفاصيل/الملاحظات (طلب {parts[2]})."
        )
        await notify_admin_dm(
            context,
            f"📝 اكتب الآن التفاصيل/الملاحظات الخاصة بطلب `{parts[2]}` (تُحفظ ليمكنك الرجوع لها لاحقاً، لا تُرسل للزبون):"
        )
        return
    ...
    if data.startswith("bot_file#"):
        ...
        await query.edit_message_text(
            f"📩 تحقق من رسائلك الخاصة مع البوت لإرسال الملف (طلب {parts[2]})."
        )
        await notify_admin_dm(
            context,
            f"📤 أرسل ملف البوت الآن هنا (كـ Document) للمستخدم `{parts[1]}` (طلب {parts[2]}):"
        )
        return
    ...
    if data.startswith("bot_pay#"):
        ...
        await context.bot.send_message(
            ADMIN_CHANNEL_ID,
            f"💰 تم دفع طلب البوت (رقم {order_id}) من قبل الزبون `{user_id}` — تم خصم ${price:.2f}"
        )
        return
